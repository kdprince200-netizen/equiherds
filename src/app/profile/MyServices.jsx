...
import { Eye, Star, Home, User, Calendar } from "lucide-react";
import { toast } from "react-hot-toast";
import { Pagination } from "antd";

export default function MyServices() {
  const [activeTab, setActiveTab] = useState("stable"); // "stable", "trainer", "otherservice", or "horse"
  ...
  const [horseBookings, setHorseBookings] = useState([]);
  const [allHorseBookings, setAllHorseBookings] = useState([]);
  ...
  const fetchMyBookings = async () => {
    try {
      ...
      if (activeTab === "stable") {
        ...
        // For horse bookings, fetch all once and paginate client-side
        // Only fetch if we don't have all bookings or if page is 1 (tab change)
        if (pagination.page === 1 || allHorseBookings.length === 0) {
          response = await getRequest(
            `/api/bookingHorses?userId=${userData.id}`
          );
          if (response.success && response.data) {
            const allBookings = response.data || [];
            setAllHorseBookings(allBookings);
            ...
            setHorseBookings(paginatedBookings);
            setPagination(prev => ({
              ...prev,
              total: allBookings.length,
              pages: Math.ceil(allBookings.length / prev.limit)
            }));
          } else {
            setAllHorseBookings([]);
            setHorseBookings([]);
            ...
            setError("Failed to fetch horse bookings");
          }
        } else {
          // Just paginate existing data
          const startIndex = (pagination.page - 1) * pagination.limit;
          const endIndex = startIndex + pagination.limit;
          const paginatedBookings = allHorseBookings.slice(startIndex, endIndex);
          
          setHorseBookings(paginatedBookings);
          setPagination(prev => ({
            ...prev,
            total: allHorseBookings.length,
            pages: Math.ceil(allHorseBookings.length / prev.limit)
          }));
        }
      } else {
        response = await getRequest(
          `/api/bookingOtherServices?clientId=${userData.id}&page=${pagination.page}&limit=${pagination.limit}`
        );
        if (response.success && response.data) {
  ...
  const getServiceType = (bookingType, serviceType) => {
    ...
    } else if (serviceType === 'horse') {
      return 'Horse Appointment';
    } else {
  ...
  const handleRatingClick = (booking) => {
    ...
    if (booking.ratingUserId || booking.ratinguserid) {
      return;
    }
    setSelectedBooking(booking);
  ...
  const handleRatingSubmit = async () => {
    ...
    try {
      ...
      if (activeTab === "stable") {
        ...
        if (!selectedBooking.serviceId?._id) {
          toast.error('Other service information not available');
          setRatingLoading(false);
          return;
        }
        response = await postRequest('/api/otherServices/rating', {
          otherServiceId: selectedBooking.serviceId._id,
          rating: rating
        });
      }
      ...
      if (response.success) {
        ...
        try {
          const apiEndpoint = activeTab === "stable" 
            ...
            : `/api/bookingOtherServices/${selectedBooking._id}`;
          ...
          if (bookingUpdateResponse.success) {
            ...
            if (activeTab === "stable") {
              setStableBookings(prev => prev.map(booking => 
                booking._id === selectedBooking._id 
                  ? { ...booking, ratingUserId: userData.id }
                  : booking
              ));
            } else if (activeTab === "trainer") {
              ...
              setOtherServiceBookings(prev => prev.map(booking => 
                booking._id === selectedBooking._id 
                  ? { ...booking, ratingUserId: userData.id }
                  : booking
              ));
        ...
        <nav className="-mb-px flex space-x-8">
          ...
          <button
            onClick={() => {
              setActiveTab("horse");
              setPagination(prev => ({ ...prev, page: 1 }));
              setAllHorseBookings([]); // Reset to force refetch
            }}
            className={`py-2 px-1 border-b-2 font-medium text-sm flex items-center gap-2 transition-colors ${
              activeTab === "horse"
                ? "border-brand text-brand"
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead className="bg-[color:var(--primary)]/10 text-brand">
                  <tr>
                    <th className="px-3 py-3 text-left font-medium min-w-[120px]">{activeTab === "horse" ? "Horse Name" : "Service Type"}</th>
                    <th className="px-3 py-3 text-left font-medium min-w-[100px]">{activeTab === "horse" ? "Appointment Date" : "Start Date"}</th>
                    <th className="px-3 py-3 text-left font-medium min-w-[100px]">{activeTab === "horse" ? "Visit Time" : "End Date"}</th>
                    <th className="px-3 py-3 text-left font-medium min-w-[80px]">Status</th>
                    <th className="px-3 py-3 text-left font-medium min-w-[90px]">{activeTab === "horse" ? "Quotation" : "Price"}</th>
                ...
                <tbody>
                  {currentBookings.length === 0 ? (
                    ...
                    currentBookings.map((booking) => {
                      // For horse bookings, use appointmentDate and status field
                      const status = activeTab === "horse" 
                        ? (booking.status || "Pending")
                        : getStatus(booking.startDate, booking.endDate);
                      const startDate = activeTab === "horse" ? booking.appointmentDate : booking.startDate;
                      const endDate = activeTab === "horse" ? booking.appointmentDate : booking.endDate;
                      const price = activeTab === "horse" ? booking.quotationAmount : booking.totalPrice;
                        ...
                        <tr key={booking._id} className="border-t border-[color:var(--primary)]/20">
                          <td className="px-3 py-3 font-medium">
                            <div className="max-w-[120px] truncate" title={getServiceType(booking.bookingType, activeTab)}>
                              {activeTab === "horse" 
                                ? (booking.horseId?.horseName || 'Horse Appointment')
                                : getServiceType(booking.bookingType, activeTab)}
                            </div>
                          </td>
                          <td className="px-3 py-3">
                            <div className="max-w-[100px] truncate" title={formatDate(startDate)}>
                              {formatDate(startDate)}
                            </div>
                          </td>
                          <td className="px-3 py-3">
                            <div className="max-w-[100px] truncate" title={activeTab === "horse" ? booking.visitTime : formatDate(endDate)}>
                              {activeTab === "horse" ? booking.visitTime : formatDate(endDate)}
                            </div>
                          </td>
                          <td className="px-3 py-3">
                          ...
                          <td className="px-3 py-3 font-medium text-right">
                            <div className="max-w-[90px] truncate" title={formatPrice(price)}>
                              {formatPrice(price)}
                            </div>
                          </td>
                          <td className="px-3 py-3 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {activeTab !== "horse" && (
                                <button
                                  onClick={() => !(booking.ratingUserId || booking.ratinguserid) && handleRatingClick(booking)}
                                  className={`inline-flex items-center justify-center w-8 h-8 rounded-full transition-colors ${
                                    (booking.ratingUserId || booking.ratinguserid)
                                      ? "text-yellow-500 cursor-default" 
                                      : "text-yellow-600 hover:text-yellow-700 hover:bg-yellow-50"
                                  }`}
                                  title={(booking.ratingUserId || booking.ratinguserid) ? "Already Rated" : "Rate Service"}
                                  disabled={!!(booking.ratingUserId || booking.ratinguserid)}
                                >
                                  <Star 
                                    size={16} 
                                    className={(booking.ratingUserId || booking.ratinguserid) ? "fill-yellow-500" : ""}
                                  />
                                </button>
                              )}
                            </div>
                ...
                return (
                  <div key={booking._id} className="bg-white rounded-lg border border-[color:var(--primary)]/20 p-4 shadow-sm">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {activeTab === "stable" 
                            ...
                            : activeTab === "horse"
                            ? (booking.horseId?.breed || 'Unknown Breed')
                            : (booking.serviceId?.title || 'Unknown Service Provider')
                          }
                        </p>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide">{activeTab === "horse" ? "Appointment Date" : "Start Date"}</p>
                        <p className="text-sm text-gray-900">
                          {formatDate(startDate)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 uppercase tracking-wide">{activeTab === "horse" ? "Visit Time" : "End Date"}</p>
                        <p className="text-sm text-gray-900">
                          {activeTab === "horse" ? booking.visitTime : formatDate(endDate)}
                        </p>
                      </div>
                    ...
                    <div className="pt-3 border-t border-gray-200">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-700">{activeTab === "horse" ? "Quotation Amount" : "Total Price"}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-lg font-bold text-brand">{formatPrice(price)}</span>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2">
                      {activeTab === "horse" ? "Seller Information" : "Service Provider"}
                    </h4>
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-gray-900">
                          {activeTab === "horse"
                            ? (`${selectedBooking.sellerId?.firstName || ''} ${selectedBooking.sellerId?.lastName || ''}`.trim() || 'Unknown Seller')
                      ...
                      <div>
                        ...
                        <p className="text-gray-900">
                          {activeTab === "horse"
                            ? (selectedBooking.sellerId?.email || 'N/A')
                  ...
                  <div className="space-y-4">
                    <h4 className="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2">
                      {activeTab === "horse" ? "Appointment Details" : "Booking Dates"}
                    </h4>
                    <div className="space-y-3">
                      {activeTab === "horse" ? (
                        <>
                          ...
                          <div>
                            ...
                            <p className="text-gray-900">{selectedBooking.visitTime || 'N/A'}</p>
                          </div>
                        </>
                      ) : (
                        <>
                      ...
                      {activeTab === "horse" ? (
                        ...
                        <>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              {(selectedBooking.ratingUserId || selectedBooking.ratinguserid) ? (
                                <>
                      ...
                      {activeTab === "horse" ? (
                        <>
                          ...
                          <div>
                            ...
                            <p className="text-gray-900">{selectedBooking.horseId?.breed || 'Not specified'}</p>
                          </div>
                          <div>
                          ...
                          {selectedBooking.horseId?.photos && selectedBooking.horseId.photos.length > 0 && (
                            <div>
                              <label className="text-sm font-medium text-gray-600">Photos</label>
                              <div className="mt-2 flex gap-2 flex-wrap">
                                {selectedBooking.horseId.photos.slice(0, 3).map((photo, index) => (
                                  <img 
                                    key={index}
                                    src={photo} 
                        ...
                        <>
                          <div>
                            ...
                            <p className="text-gray-900">{selectedBooking.serviceId?.title || 'Unknown Service Provider'}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-600">Description</label>
                            <p className="text-gray-900">{selectedBooking.serviceId?.details || 'No description available'}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-600">Service Type</label>
                            <p className="text-gray-900">{selectedBooking.serviceId?.serviceType || 'Not specified'}</p>
                          </div>
                          <div>
                            <label className="text-sm font-medium text-gray-600">Location</label>
                            <p className="text-gray-900">{selectedBooking.serviceId?.location || 'Location not specified'}</p>
                          </div>
                          {selectedBooking.serviceId?.coordinates && (
                            <div>
                              <label className="text-sm font-medium text-gray-600">Coordinates</label>
                              <p className="text-gray-900 text-sm">
                                Lat: {selectedBooking.serviceId.coordinates.lat}, 
                                Lng: {selectedBooking.serviceId.coordinates.lng}
                              </p>
                            </div>
                          )}
                        </>
                ...
                {activeTab !== "horse" && selectedBooking.additionalServices && selectedBooking.servicePriceDetails && (
                  <div className="space-y-4">
                    <h4 className="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {activeTab === "stable" 
                    ...
                    : (selectedBooking.serviceId?.title || 'Unknown Service Provider')
                  }
                </p>
              </div>
              <button
