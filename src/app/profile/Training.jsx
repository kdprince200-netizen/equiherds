...
export default function Trainer() {
  ...
  const [trainerLimit, setTrainerLimit] = useState(0);
  ...
  const validateForm = () => {
    ...
    if (!form.images?.length && !imagePreviews?.length && !prevImages?.length) {
      errors.push("At least one image is required");
    }

    // Validate pricing fields when checkboxes are selected
    // Disciplines validation
    if (form.disciplines?.dressage && !form.disciplines?.dressagePrice) {
      errors.push("Price is required for 'Dressage' option");
    }
    if (form.disciplines?.showJumping && !form.disciplines?.showJumpingPrice) {
      errors.push("Price is required for 'Show Jumping' option");
    }
    if (form.disciplines?.eventing && !form.disciplines?.eventingPrice) {
      errors.push("Price is required for 'Eventing' option");
    }
    if (form.disciplines?.endurance && !form.disciplines?.endurancePrice) {
      errors.push("Price is required for 'Endurance' option");
    }
    if (form.disciplines?.western && !form.disciplines?.westernPrice) {
      errors.push("Price is required for 'Western' option");
    }
    if (form.disciplines?.vaulting && !form.disciplines?.vaultingPrice) {
      errors.push("Price is required for 'Vaulting' option");
    }

    // Training validation
    if (form.training?.onLocationLessons && !form.training?.onLocationLessonsPrice) {
      errors.push("Price is required for 'On Location Lessons' option");
    }
    if (form.training?.lessonsOnTrainersLocation && !form.training?.lessonsOnTrainersLocationPrice) {
      errors.push("Price is required for 'Lessons on Trainer's Location' option");
    }

    // Competition Coaching validation
    if (form.competitionCoaching?.onLocationCoaching && !form.competitionCoaching?.onLocationCoachingPrice) {
      errors.push("Price is required for 'On Location Coaching' option");
    }
    ...
    validatePrice(form.disciplines?.dressagePrice, "Dressage price");
    validatePrice(form.disciplines?.showJumpingPrice, "Show Jumping price");
    validatePrice(form.disciplines?.eventingPrice, "Eventing price");
    validatePrice(form.disciplines?.endurancePrice, "Endurance price");
    validatePrice(form.disciplines?.westernPrice, "Western price");
    validatePrice(form.disciplines?.vaultingPrice, "Vaulting price");
    validatePrice(form.training?.onLocationLessonsPrice, "On Location Lessons price");
    validatePrice(form.training?.lessonsOnTrainersLocationPrice, "Lessons on Trainer's Location price");
    validatePrice(form.competitionCoaching?.onLocationCoachingPrice, "On Location Coaching price");
  ...
  const handleAddStable = async (e) => {
    ...
    // Check subscription limits for new trainers (not for editing)
    // Skip validation for superAdmin
    if (!editingId && !isSuperAdmin) {
      if (!subscriptionDetails) {
        toast.error("You need an active subscription to add trainers. Please subscribe first.");
        return;
      }
      if (stables.length >= trainerLimit) {
        toast.error("Your trainer limit is full. Please upgrade your subscription to add more trainers.");
        return;
      }
    }
    ...
    try {
      ...
      if (
        ...
        toast.error(editingId ? "Failed to update trainer. Please try again." : "Failed to create trainer. Please try again.");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  const fetchUserSubscriptionDetails = async () => {
    try {
      ...
      if (userResponse?.user) {
        ...
        if (isAdmin) {
          setTrainerLimit(999999); // Set a very high limit for superAdmin
          return;
        }
        ...
        if (userData.subscriptionId) {
          ...
          if (subscriptionResponse?.subscription) {
            ...
            // Get trainer limit from subscription description
            const trainerLimitFromSub = parseInt(subscription.description?.Trainer || "0");
            setTrainerLimit(trainerLimitFromSub);
          }
        } else {
    ...
    async function loadByUser() {
      ...
      try {
        ...
        if (Array.isArray(list)) {
          ...
          // After loading trainers, check subscription limits
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          className={`px-4 py-2 rounded font-medium transition ${
            (!isSuperAdmin && (stables.length >= trainerLimit || !subscriptionDetails))
              ? 'bg-gray-400 !text-white cursor-not-allowed' 
              : 'bg-[color:var(--primary)] !text-white hover:bg-[color:var(--primary)]/90 cursor-pointer'
          }`}
          disabled={!isSuperAdmin && (stables.length >= trainerLimit || !subscriptionDetails)}
          onClick={() => {
            // Skip validation for superAdmin
            if (!isSuperAdmin) {
              if (!subscriptionDetails) {
              ...
              // Check if current trainer count has reached the limit
              if (stables.length >= trainerLimit) {
                toast.error("Your trainer limit is full. Please upgrade your subscription to add more trainers.");
                return;
              }
        ...
        >
          {isSuperAdmin ? "+ Add New" : !subscriptionDetails ? "Get Subscription" : (stables.length >= trainerLimit) ? "Your Limit is Full" : "+ Add New"}
        </button>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-blue-600">
                Trainer Limit: {stables.length} / {trainerLimit} trainers
              </p>
              {(stables.length >= trainerLimit) && (
                <p className="text-sm text-red-600 font-medium mt-1">
                  You've reached your trainer limit for this plan.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-green-600">
                You have unlimited access to add trainers without subscription limits.
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-yellow-600">
                You need an active subscription to add trainers.
              </p>
            </div>
            <button
              onClick={() => {
            ...
            <form onSubmit={handleAddStable} className="space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring focus:border-[color:var(--primary)]"
                  rows={3}
                  placeholder="Describe your training services"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-brand mb-1">
              ...
              <div className="border-t pt-4">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Optional: Select disciplines and add prices as needed
                </p>
              ...
              <div className="border-t pt-4">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Optional: Select training options and add prices as needed
                </p>
              ...
              <div className="border-t pt-4">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Optional: Add your diplomas and certifications
                </p>
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onPointerDown={handleAddDiploma}
