  ...
  useEffect(() => {
    const fetchHorses = async () => {
      try {
        ...
        // Filter active horses and map them
        const allHorses = data
          ...
          .map((h) => ({
            ...
            rating: typeof h?.Rating === "number" ? h.Rating : (typeof h?.rating === "number" ? h.rating : 0),
            sponsoredType: h?.sponsoredType || "",
            isSponsored: h?.sponsoredType === "sponsored",
          }));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          {!loading && !error && horses.map((horse) => (
            <div key={horse.id} className="text-center px-4 relative">
              {/* Sponsored Badge */}
              {horse.isSponsored && (
