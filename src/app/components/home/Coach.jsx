    ...
    const fetchTrainers = async () => {
      try {
        ...
        const allTrainers = data.map((t) => ({
          ...
          sponsoredType: t?.sponsoredType || "",
          isSponsored: t?.sponsoredType === "sponsored",
        }));

        // Separate sponsored and non-sponsored
        const sponsored = allTrainers.filter(t => t.isSponsored).slice(0, 2);
        const nonSponsored = allTrainers.filter(t => !t.isSponsored);
    ...
    <section className="py-8 px-4 sm:px-8 lg:px-16 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="items-center justify-items-center p-8 text-center">
          <h2 className="text-2xl font-semibold primary mb-4"> Expert Trainers for Your Horse</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          {!loading && !error && trainers.map((trainer) => (
            <div key={trainer.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 relative">
              {/* Sponsored Badge */}
              {trainer.isSponsored && (
                <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-yellow-400 to-orange-500 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
                  ⭐ Sponsored
                </div>
              )}
              {/* Trainer Image */}
