  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const sponsored = allServices.filter(s => s.isSponsored).slice(0, 2);
        const nonSponsored = allServices.filter(s => !s.isSponsored);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          {!loading && !error && services.map((svc) => (
            <div key={svc.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 relative">
              {/* Sponsored Badge */}
              {svc.isSponsored && (
