...
export default function LocationPicker({ 
  ...
  const handleSearchResultSelect = (result) => {
    // Handle both Google Places API and Nominatim formats
    const lat = parseFloat(result.lat || result.geometry?.location?.lat);
    const lng = parseFloat(result.lon || result.geometry?.location?.lng);
    const newLocation = { lat, lng };
    
    setSelectedLocation(newLocation);
    setMapCenter(newLocation);
    
    // Use the appropriate display name based on API format
    const displayName = result.formatted_address || result.display_name || result.name;
    ...
    if (onLocationTextChange) {
      onLocationTextChange(displayName);
    }
  };
    ...
    <div className="w-full">
      ...
      <div className="relative mb-4" ref={searchRef}>
        <input
          ...
          placeholder="Search for a location (e.g., Le chêne brûlé 49490 noyant village France)..."
        ...
        {showDropdown && searchResults.length > 0 && (
          ...
          >
            {searchResults.map((result, index) => {
              // Handle Google Places API response format
              const primaryName = result.name || result.formatted_address || result.display_name;
              const locationParts = result.formatted_address ? result.formatted_address.split(', ') : [];
