...
export default function StableList() {
  ...
  // New filters
  const [shortTermStayOptions, setShortTermStayOptions] = useState([]);
  const [longTermStayOptions, setLongTermStayOptions] = useState([]);
      ...
      try {
        ...
        const normalized = Array.isArray(data) ? data.map((d) => {
          ...
          return {
            ...
            specialFacilities: d?.specialFacilities || {},
          };
        }) : [];
  ...
  useEffect(() => {
    ...
    setPriceMin(0); // Start with empty/minimum value
    setPriceMax(max);
    setMinRating(0);
    setSearch("");
    setSelectedDay("");
    setShortTermStayOptions([]);
    setLongTermStayOptions([]);
    setStallionsAccepted(false);
    setEventFacilities([]);
    setSpecialFacilities([]);
  }, [items]);
  ...
  const filtered = useMemo(() => {
    ...
    console.log('Filter values:', { shortTermStayOptions, longTermStayOptions, stallionsAccepted, eventFacilities, specialFacilities, search, minRating, priceMin, priceMax, selectedDay, useNearMe, radiusKm, userLocation });
    const base = items
    .map((s) => {
      // Pre-compute distance if needed
      let _distance = null;
      if (useNearMe && userLocation && s.coordinates && typeof s.coordinates.lat === 'number' && typeof s.coordinates.lng === 'number') {
        _distance = haversineKm(userLocation.lat, userLocation.lng, s.coordinates.lat, s.coordinates.lng);
      }
      return { ...s, _distance };
    })
    .filter((s) => {
      ...
      // New filter logic
      const matchesShortTermStay = shortTermStayOptions.length === 0 || shortTermStayOptions.some(option => 
        s.shortTermStay?.[option] === true
      );
      
      const matchesLongTermStay = longTermStayOptions.length === 0 || longTermStayOptions.some(option => 
        s.longTermStay?.[option] === true
      );
      ...
      const matchesSpecialFacilities = specialFacilities.length === 0 || specialFacilities.some(facility => 
        s.specialFacilities?.[facility] === true
      );

      // Near me filter
      const matchesNearMe = !useNearMe || (typeof s._distance === 'number' && s._distance <= radiusKm);
      
      const result = matchesSearch && matchesRating && matchesPrice && matchesDay && 
             matchesShortTermStay && matchesLongTermStay && matchesStallions && matchesEventFacilities && matchesSpecialFacilities && matchesNearMe;
    ...
    // Sort by distance if near-me enabled
    if (useNearMe) {
    ...
    return base;
  }, [items, search, minRating, priceMin, priceMax, selectedDay, shortTermStayOptions, longTermStayOptions, stallionsAccepted, eventFacilities, specialFacilities, useNearMe, radiusKm, userLocation]);
  ...
  function handleReset() {
    ...
    setSearch("");
    setMinRating(0);
    setPriceMin(0); // Reset to empty/minimum value
    setPriceMax(max);
    setSelectedDay("");
    setShortTermStayOptions([]);
    setLongTermStayOptions([]);
  ...
  // Check if user can book (must be buyer account type)
  const canUserBook = () => {
    return userData && userData.accountType === "buyer";
  };
        ...
        <div className="sticky top-24 rounded-xl border border-gray-200 bg-white/60 p-5 shadow-sm backdrop-blur">
          ...
          <div className="mb-4">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">{priceMin === 0 ? 'Any' : `€${priceMin}`} - €{priceMax}</span>
            </div>
          ...
          <div className="mb-6">
            <label className="mb-2 block text-sm font-medium text-gray-700">Short Term Stay</label>
            <Checkbox.Group
              value={shortTermStayOptions}
              onChange={(checkedValues) => setShortTermStayOptions(checkedValues)}
              className="flex flex-col space-y-2"
            >
              {[
                { key: 'inStableStraw', label: 'In Stable (Straw)' },
                { key: 'inStableShavings', label: 'In Stable (Shavings)' },
                { key: 'inFieldAlone', label: 'In Field (Alone)' },
                { key: 'inFieldHerd', label: 'In Field (Herd)' }
              ].map((option) => (
                <Checkbox
                  key={option.key}
                  value={option.key}
          ...
          <div className="mb-6">
            <label className="mb-2 block text-sm font-medium text-gray-700">Long Term Stay</label>
            <Checkbox.Group
              value={longTermStayOptions}
              onChange={(checkedValues) => setLongTermStayOptions(checkedValues)}
              className="flex flex-col space-y-2"
            >
              {[
                { key: 'inStableStraw', label: 'In Stable (Straw)' },
          ...
          <div className="mb-4">
            <label className="mb-2 block text-sm font-medium text-gray-700">Special Facilities</label>
            <Checkbox.Group
              value={specialFacilities}
              onChange={(checkedValues) => setSpecialFacilities(checkedValues)}
              className="flex flex-col space-y-2"
            >
              {[
                ...
                { key: 'lungingArena', label: 'Lunging Arena' }
              ].map((facility) => (
                <Checkbox
                  key={facility.key}
                  value={facility.key}
              ...
              <div className="lg:w-1/3 p-6 overflow-y-auto max-h-[400px] lg:max-h-[500px]">
                <div className="space-y-4">
                  ...
                  {selectedStable.stallionsAccepted && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-800 mb-2">Accommodation for the riders</h4>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <span className="text-sm text-gray-700">Accommodation for the riders</span>
                  ...
                  {/* Special Facilities */}
                  {selectedStable.specialFacilities && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-800 mb-3">Special Facilities</h4>
                      <div className="space-y-2">
                        {selectedStable.specialFacilities.indoorArena && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-700">Indoor Arena</span>
                            {selectedStable.specialFacilities.indoorArenaPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.indoorArenaPrice}</span>
                            )}
                          </div>
                        )}
                        {selectedStable.specialFacilities.walker && (
                          <div className="flex justify-between items-center">
                            ...
                            {selectedStable.specialFacilities.walkerPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.walkerPrice}</span>
                            )}
                          </div>
                        )}
                        {selectedStable.specialFacilities.treadmill && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-700">Treadmill</span>
                            {selectedStable.specialFacilities.treadmillPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.treadmillPrice}</span>
                            )}
                          </div>
                        )}
                        {selectedStable.specialFacilities.spa && (
                          <div className="flex justify-between items-center">
                            ...
                            {selectedStable.specialFacilities.spaPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.spaPrice}</span>
                            )}
                          </div>
                        )}
                        {selectedStable.specialFacilities.solarium && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-700">Solarium</span>
                            {selectedStable.specialFacilities.solariumPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.solariumPrice}</span>
                            )}
                          </div>
                        )}
                        {selectedStable.specialFacilities.lungingArena && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-700">Lunging Arena</span>
                            {selectedStable.specialFacilities.lungingArenaPrice && (
                              <span className="text-sm font-medium text-brand">€{selectedStable.specialFacilities.lungingArenaPrice}</span>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col gap-3">
                      {canUserBook() ? (
                        ...
                        <button 
                          ...
                          title={userData ? "Only buyers can book stables" : "Please login to book"}
                        >
                          {userData ? "Only Buyers Can Book" : "Login to Book"}
                        </button>
                      )}
                      <button 
                        onClick={closeModal} 
