  ...
  try {
    ...
    if (timeSlots && Array.isArray(timeSlots)) {
      for (const slot of timeSlots) {
        if (!slot.date || !slot.startTime || !slot.endTime || !slot.day) {
          return NextResponse.json(
            { success: false, message: "Each time slot must have date, startTime, endTime, and day" },
            { status: 400 }
          );
        }
    ...
    // Check for booking conflicts if time slots are provided
    if (timeSlots && timeSlots.length > 0) {
      for (const slot of timeSlots) {
        const slotDate = new Date(slot.date);
        const existingBookings = await BookingTrainer.find({
          trainerId: trainerId,
          ...
          $or: [
            ...
            },
            {
              ...
              'timeSlots.endTime': slot.endTime
            }
          ]
        });
        ...
        if (existingBookings.length > 0) {
          return NextResponse.json(
            { 
              success: false, 
              message: `Time slot conflict: ${slot.date} ${slot.startTime}-${slot.endTime} is already booked` 
            },
            { status: 409 }
          );
        }
