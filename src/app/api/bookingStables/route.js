...
export async function POST(req) {
  ...
  try {
    ...
    if (bookingStatus && !["pending", "confirmed", "cancelled", "completed"].includes(bookingStatus)) {
      return NextResponse.json(
        { success: false, message: "Invalid booking status. Must be 'pending', 'confirmed', 'cancelled', or 'completed'" },
        { status: 400 }
      );
    }
    ...
    const bookingData = {
      ...
      bookingStatus: bookingStatus || 'pending', // Default to 'pending' if not provided
      paymentId: paymentId || null, // Optional field
