...
export async function GET(req) {
  ...
  try {
    ...
    const trainers = await Trainer.find(query).populate({ path: "userId", select: "firstName lastName email subscriptionExpiry accountType" });
    
    // Add status to each trainer's userId field and filter only active sellers
    const trainersWithStatus = trainers.map(trainer => {
      if (trainer.userId) {
        // Check if user is superAdmin - always active
        if (trainer.userId.accountType === "superAdmin") {
          return {
            ...trainer.toObject(),
            userId: {
              ...trainer.userId.toObject(),
              status: "active"
            }
          };
        }
        ...
        if (trainer.userId.subscriptionExpiry) {
          const expiryDate = new Date(trainer.userId.subscriptionExpiry);
      ...
      // Only return trainers where userId exists and status is active
      return trainer.userId && trainer.userId.status === "active";
    });
    
    return NextResponse.json(trainersWithStatus, { status: 200 });
  } catch (error) {
