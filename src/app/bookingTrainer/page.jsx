...
function BookingTrainerContent() {
  ...
  const handleDateRangeChange = (dates) => {
    ...
    // Reset time slots when date range changes
  ...
  const handleTimeSlotChange = (time, timeType) => {
    if (timeType === 'start') {
      setStartTime(time);
  ...
  // Generate available time slots based on trainer schedule and selected dates
  const generateAvailableTimeSlots = () => {
    if (!selectedTrainer || !selectedTrainer.schedule || !selectedDateRange) {
      return [];
    }
    ...
    while (currentDate.isSameOrBefore(endDate)) {
      ...
      // Find trainer's schedule for this day
      const daySchedule = selectedTrainer.schedule.find(slot => 
  ...
  // Handle time slot selection for specific dates
  const handleTimeSlotSelection = (slot, checked) => {
    if (checked) {
      setSelectedTimeSlots(prev => [...prev, slot]);
    } else {
      setSelectedTimeSlots(prev => prev.filter(s => 
          ...
          <Col xs={24}>
            <Card title="Booking Information">
              ...
              >
                  ...
                  >
                    <div className="flex gap-2">
                      <TimePicker
                        ...
                        size="large"
                        style={{ width: '50%' }}
                        onChange={(time) => handleTimeSlotChange(time ? time.format('HH:mm') : null, 'start')}
                      ...
                      <TimePicker
                        ...
                        size="large"
                        style={{ width: '50%' }}
                        onChange={(time) => handleTimeSlotChange(time ? time.format('HH:mm') : null, 'end')}
                ...
                {/* Time Slot Selection */}
                {selectedDateRange && selectedTrainer && (
                  <div className="space-y-4">
                    <h4 className="text-lg font-semibold text-gray-800 border-b pb-2">Available Time Slots</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {generateAvailableTimeSlots().map((slot, index) => (
                        <div key={index} className="border rounded-lg p-3 bg-gray-50">
                          ...
                          <Checkbox
                            checked={selectedTimeSlots.some(s => 
                              s.date === slot.date && s.startTime === slot.startTime
                            )}
                            onChange={(e) => handleTimeSlotSelection(slot, e.target.checked)}
                          >
                            <span className="text-sm">Select this slot</span>
                          </Checkbox>
                        </div>
                      ))}
                    </div>
                    {selectedTimeSlots.length > 0 && (
                      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <p className="text-sm text-blue-800">
                          <strong>Selected Slots:</strong> {selectedTimeSlots.length} time slot(s) selected
                        </p>
                      </div>
                    )}
                  </div>
                ...
                {bookingType && selectedDateRange && (
                  <div className="mb-4 p-4 bg-brand/5 rounded-lg border border-brand/20 mt-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {selectedTimeSlots.length > 0 && (
                        <div className="flex justify-between">
                          <span className="text-gray-600">Selected Time Slots:</span>
                ...
                <Form.Item>
                  ...
                  >
                    Save Card & Create Booking (€{calculateTotalPrice().toFixed(2)})
                  </Button>
                </Form.Item>
              </Form>
            </Card>
