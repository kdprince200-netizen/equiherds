...
function BookingStablesContent() {
          ...
          try {
            ...
            if (response && response._id) {
              ...
              const normalizedData = {
                ...
                specialFacilities: response.specialFacilities || {
                  indoorArena: false,
                  walker: false,
                  treadmill: false,
                  spa: false,
  ...
  const handleServiceChange = (serviceType, serviceName, checked) => {
    setSelectedServices(prev => {
      ...
      if (serviceType === 'shortTermStay' && serviceName === 'selected') {
        ...
        // Update the specific service (for eventPricing, specialFacilities)
  ...
  const getAdditionalServiceCosts = () => {
    ...
    // Special facilities costs (per day)
    if (selectedServices.specialFacilities.indoorArena && selectedStable.specialFacilities.indoorArena) {
      totalAdditionalCost += (selectedStable.specialFacilities.indoorArenaPrice || 0) * numberOfDays;
    }
    if (selectedServices.specialFacilities.walker && selectedStable.specialFacilities.walker) {
      totalAdditionalCost += (selectedStable.specialFacilities.walkerPrice || 0) * numberOfDays;
    }
    if (selectedServices.specialFacilities.treadmill && selectedStable.specialFacilities.treadmill) {
      totalAdditionalCost += (selectedStable.specialFacilities.treadmillPrice || 0) * numberOfDays;
    }
    if (selectedServices.specialFacilities.spa && selectedStable.specialFacilities.spa) {
      totalAdditionalCost += (selectedStable.specialFacilities.spaPrice || 0) * numberOfDays;
    }
    if (selectedServices.specialFacilities.solarium && selectedStable.specialFacilities.solarium) {
      totalAdditionalCost += (selectedStable.specialFacilities.solariumPrice || 0) * numberOfDays;
    }
    if (selectedServices.specialFacilities.lungingArena && selectedStable.specialFacilities.lungingArena) {
      totalAdditionalCost += (selectedStable.specialFacilities.lungingArenaPrice || 0) * numberOfDays;
    }
  ...
  const getServicePriceDetails = () => {
    ...
    const priceDetails = {
      ...
      specialFacilities: {
        indoorArena: selectedServices.specialFacilities.indoorArena,
        indoorArenaPrice: 0,
        walker: selectedServices.specialFacilities.walker,
        walkerPrice: 0,
        treadmill: selectedServices.specialFacilities.treadmill,
        treadmillPrice: 0,
        spa: selectedServices.specialFacilities.spa,
        spaPrice: 0,
        solarium: selectedServices.specialFacilities.solarium,
        solariumPrice: 0,
        lungingArena: selectedServices.specialFacilities.lungingArena,
        lungingArenaPrice: 0
      }
    };
    ...
    // Calculate special facilities prices
    if (selectedServices.specialFacilities.indoorArena && selectedStable.specialFacilities.indoorArena) {
      const pricePerDay = selectedStable.specialFacilities.indoorArenaPrice || 0;
      priceDetails.specialFacilities.indoorArenaPrice = pricePerDay * numberOfDays;
    }
    if (selectedServices.specialFacilities.walker && selectedStable.specialFacilities.walker) {
      const pricePerDay = selectedStable.specialFacilities.walkerPrice || 0;
      priceDetails.specialFacilities.walkerPrice = pricePerDay * numberOfDays;
    }
    if (selectedServices.specialFacilities.treadmill && selectedStable.specialFacilities.treadmill) {
      const pricePerDay = selectedStable.specialFacilities.treadmillPrice || 0;
      priceDetails.specialFacilities.treadmillPrice = pricePerDay * numberOfDays;
    }
    if (selectedServices.specialFacilities.spa && selectedStable.specialFacilities.spa) {
      const pricePerDay = selectedStable.specialFacilities.spaPrice || 0;
      priceDetails.specialFacilities.spaPrice = pricePerDay * numberOfDays;
    }
    if (selectedServices.specialFacilities.solarium && selectedStable.specialFacilities.solarium) {
      const pricePerDay = selectedStable.specialFacilities.solariumPrice || 0;
      priceDetails.specialFacilities.solariumPrice = pricePerDay * numberOfDays;
    }
    if (selectedServices.specialFacilities.lungingArena && selectedStable.specialFacilities.lungingArena) {
      const pricePerDay = selectedStable.specialFacilities.lungingArenaPrice || 0;
      priceDetails.specialFacilities.lungingArenaPrice = pricePerDay * numberOfDays;
    }
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const fixedAdditionalServices = {
        ...
        specialFacilities: selectedServices.specialFacilities
      };
              ...
              {selectedStable && (
                <div className="space-y-6">
                  ...
                  <Row gutter={[16, 16]}>
                    ...
                    {selectedStable.stallionsAccepted && (
                      <Col xs={24} lg={12}>
                        <div className="border rounded-lg p-4 bg-yellow-50 h-full">
                          <h5 className="font-semibold text-yellow-800 mb-3">Accommodation for the riders</h5>
                          <div className="flex items-center justify-between">
                            <Checkbox
                              checked={selectedServices.stallionsAccepted}
                              onChange={(e) => handleServiceChange('stallionsAccepted', null, e.target.checked)}
                            >
                              <span className="text-sm">Accommodation for the riders</span>
                            </Checkbox>
                    ...
                    {/* Special Facilities */}
                    {(selectedStable.specialFacilities && Object.values(selectedStable.specialFacilities).some(option => option)) && (
                      <Col xs={24} lg={12}>
                        <div className="border rounded-lg p-4 bg-orange-50 h-full">
                          <h5 className="font-semibold text-orange-800 mb-3">Special Facilities (Per Day)</h5>
                          <div className="space-y-2">
                            {selectedStable.specialFacilities.indoorArena && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.indoorArena}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'indoorArena', e.target.checked)}
                                >
                                  <span className="text-sm">Indoor Arena</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.indoorArenaPrice}/day
                                </span>
                              </div>
                            )}
                            {selectedStable.specialFacilities.walker && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.walker}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'walker', e.target.checked)}
                                >
                                  <span className="text-sm">Walker</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.walkerPrice}/day
                                </span>
                              </div>
                            )}
                            {selectedStable.specialFacilities.treadmill && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.treadmill}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'treadmill', e.target.checked)}
                                >
                                  <span className="text-sm">Treadmill</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.treadmillPrice}/day
                                </span>
                              </div>
                            )}
                            {selectedStable.specialFacilities.spa && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.spa}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'spa', e.target.checked)}
                                >
                                  <span className="text-sm">Spa</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.spaPrice}/day
                                </span>
                              </div>
                            )}
                            {selectedStable.specialFacilities.solarium && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.solarium}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'solarium', e.target.checked)}
                                >
                                  <span className="text-sm">Solarium</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.solariumPrice}/day
                                </span>
                              </div>
                            )}
                            {selectedStable.specialFacilities.lungingArena && (
                              <div className="flex items-center justify-between">
                                <Checkbox
                                  checked={selectedServices.specialFacilities.lungingArena}
                                  onChange={(e) => handleServiceChange('specialFacilities', 'lungingArena', e.target.checked)}
                                >
                                  <span className="text-sm">Lunging Arena</span>
                                </Checkbox>
                                <span className="text-sm font-medium text-orange-700">
                                  €{selectedStable.specialFacilities.lungingArenaPrice}/day
                                </span>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {getAdditionalServiceCosts() > 0 && (
                      <div className="pt-2 border-t border-gray-200">
                        ...
                        {selectedServices.specialFacilities.indoorArena && selectedStable.specialFacilities.indoorArena && (
                          <div className="flex justify-between text-sm">
                            <span>Indoor Arena × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.indoorArenaPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
                        {selectedServices.specialFacilities.walker && selectedStable.specialFacilities.walker && (
                          <div className="flex justify-between text-sm">
                            <span>Walker × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.walkerPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
                        {selectedServices.specialFacilities.treadmill && selectedStable.specialFacilities.treadmill && (
                          <div className="flex justify-between text-sm">
                            <span>Treadmill × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.treadmillPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
                        {selectedServices.specialFacilities.spa && selectedStable.specialFacilities.spa && (
                          <div className="flex justify-between text-sm">
                            <span>Spa × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.spaPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
                        {selectedServices.specialFacilities.solarium && selectedStable.specialFacilities.solarium && (
                          <div className="flex justify-between text-sm">
                            <span>Solarium × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.solariumPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
                        {selectedServices.specialFacilities.lungingArena && selectedStable.specialFacilities.lungingArena && (
                          <div className="flex justify-between text-sm">
                            <span>Lunging Arena × {getNumberOfDays()} days:</span>
                            <span>€{(selectedStable.specialFacilities.lungingArenaPrice || 0) * getNumberOfDays()}</span>
                          </div>
                        )}
